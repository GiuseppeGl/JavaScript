...
const concat = function (str1, str2) {
  str1.slice(0, 1).concat(str2.slice(-3))
}
...
const arrayCasuale =  genArrayCaso() ;
...
let arrMisto = [5, 6, 8, 8, 4, 9, 7, 3];

function numeripari (arrparo) {
let valoriPari = arrparo.filter(numero => numero % 2 === 0);
return valoriPari;

}


let valoriPari = numeripari(arrMisto);
...
function sommaArrmisto (som) {

let somma = 0;
for(let i = 0; i < som.length; i++) {
  if (typeof som[i] === 'number' ) {
    somma += som[i];
  }
}
...
let sommaFinale = sommaArrmisto(arrMisto);
...
function sommaArrRed (sum) {

  let somma = sum.reduce((acc, value) => acc + value, 0);

  return somma;
}

let sommafinale2 = sommaArrRed(arrMisto);
console.log(sommafinale2);


/* ESERCIZIO 6 (map)
  Scrivi una funzione che, 
  dato un array di soli numeri e un numero n come parametri, 
...
console.log("il film più vecchio è " + filmVecchio + " uscito nel " + vecchio);
...
/* ESERCIZIO 12 (filter)
  Scrivi una funzione per ottenere dall'array fornito solamente
   i film usciti nel millennio corrente.
*/
